...
type AppLogFunc func(lvl LOG.Level, f string, args ...interface{})
...
func NewDiskQueue(name string, dataPath string, maxBytesPerFile int64,
	...
	if err != nil && !os.IsNotExist(err) {
		d.logf(LOG.ERROR, "DISKQUEUE(%s) failed to retrieveMetaData - %s", d.name, err)
	}
...
func (d *DiskQueue) Delete() error {
	if err := d.deleteAllFiles(); err != nil {
		d.logf(LOG.ERROR, "DISKQUEUE(%s) failed to remove files - %s", d.name, err)
	}
	return d.exit(true)
}
...
func (d *DiskQueue) exit(deleted bool) error {
	...
	if deleted {
		d.logf(LOG.INFO, "DISKQUEUE(%s): deleting", d.name)
	} else {
		d.logf(LOG.INFO, "DISKQUEUE(%s): closing", d.name)
	}
...
func (d *DiskQueue) Empty() error {
	...
	d.logf(LOG.INFO, "DISKQUEUE(%s): emptying", d.name)
...
func (d *DiskQueue) deleteAllFiles() error {
	...
	if innerErr != nil && !os.IsNotExist(innerErr) {
		d.logf(LOG.ERROR, "DISKQUEUE(%s) failed to remove metadata file - %s", d.name, innerErr)
		return innerErr
	}
...
func (d *DiskQueue) skipToNextRWFile() error {
	...
	for i := d.readFileNum; i <= d.writeFileNum; i++ {
		...
		if innerErr != nil && !os.IsNotExist(innerErr) {
			d.logf(LOG.ERROR, "DISKQUEUE(%s) failed to remove data file - %s", d.name, innerErr)
			err = innerErr
		}
	}
...
func (d *DiskQueue) readOne() ([]byte, error) {
	...
	if d.readFile == nil {
		...
		d.logf(LOG.INFO, "DISKQUEUE(%s): readOne() opened %s", d.name, curFileName)
...
func (d *DiskQueue) writeOne(data []byte) error {
	...
	if d.writeFile == nil {
		...
		d.logf(LOG.INFO, "DISKQUEUE(%s): writeOne() opened %s", d.name, curFileName)
	...
	if d.writePos > d.maxBytesPerFile {
		...
		err = d.sync()
		if err != nil {
			d.logf(LOG.ERROR, "DISKQUEUE(%s) failed to sync - %s", d.name, err)
		}
...
func (d *DiskQueue) moveForward() {
	...
	if oldReadFileNum != d.nextReadFileNum {
		...
		err := os.Remove(fn)
		if err != nil {
			d.logf(LOG.ERROR, "DISKQUEUE(%s) failed to Remove(%s) - %s", d.name, fn, err)
		}
	}
	...
	for {
		...
		if (d.readFileNum < d.writeFileNum) || (d.readPos < d.writePos) {
			if d.nextReadPos == d.readPos {
				...
				if err != nil {
					d.logf(LOG.ERROR, "DISKQUEUE(%s) reading at %d of %s - %s",
...
exit:
	d.logf(LOG.INFO, "DISKQUEUE(%s): closing ... ioLoop", d.name)
	syncTicker.Stop()
	d.exitSyncChan <- 1
}
