...
func TestTransformLog(t *testing.T) {
	...
	var nr int
	{
		...
		ref := mgo.DBRef{Collection: "fcol1", Id: "id1", Database: "fdb1"}

		logs := []*OplogRecord{
			mockTransLogs("i", "fdb1.fcol1", bson.M{"a": 1}),
			mockTransLogs("i", "fdb2.fcol2", bson.M{"a": 1, "b": ref}),
		...
		assert.Equal(t, mockTransLogs("i", "fdb2.fcol2", bson.M{"a": 1, "b": ref}), logs[1], "should be equal")
	...
	}
	{
		...
		logs := []*OplogRecord{
			...
			mockTransLogs("c", "admin.$cmd", bson.M{"applyOps": []interface{}{
				...
				bson.M{"op": "i", "ns": "fdb1.fcol2", "o": bson.M{"b": ref}}}}),
		}
		logs = transformLogs(logs, nsTrans, true)
